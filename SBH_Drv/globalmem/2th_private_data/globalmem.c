...
static void globalmem_setup_cdev(struct globalmem_dev *dev, int minor)
{
...
static int globalmem_init(void)
{
	...
	pdev = kmalloc(sizeof(struct globalmem_dev), GFP_KERNEL);
	if (pdev == NULL)
	{
		result = -ENOMEM;
		goto malloc_dev_error;
	}
	memset(pdev, 0, sizeof(struct globalmem_dev));
	globalmem_setup_cdev(pdev, 0);	
	return 0;
malloc_dev_error:
