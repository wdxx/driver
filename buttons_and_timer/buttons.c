...
static struct pin_desc pins_desc[4] = {
	{S3C2410_GPF(0), &press_cnt[0]},
	{S3C2410_GPF(1), &press_cnt[1]},
	{S3C2410_GPF(2), &press_cnt[2]},
	{S3C2410_GPF(4), &press_cnt[3]},
};
...
static struct pin_desc *p_desc;
irqreturn_t tq2440_buttons_irq(int irq, void *dev_id)
{
	p_desc = (struct pin_desc *)dev_id;
...
int tq_buttons_release(struct inode *inode, struct file *fp)
{
	...
	for (i=0; i < sizeof(button_irqs)/sizeof(button_irqs[0]); i++)
		free_irq(button_irqs[i].irq, (void *)&pins_desc[i]);
...
static void button_timer_function(unsigned long dat)
{
	...
	if (!p_desc)
		return;
	pinval = s3c2410_gpio_getpin(p_desc->pin);
...
int tq_buttons_open(struct inode *inode, struct file *fp)
{
	...
	/* 申请按键所用中断号 */
	err = request_irq(button_irqs[0].irq, tq2440_buttons_irq, button_irqs[0].flags, button_irqs[0].name, &pins_desc[0]);
	if (err < 0){
		printk("failed request irq_eint0 on %s,in line:%d\n", __FUNCTION__, __LINE__);
		goto request_irq_eint0_error;
	}
	err = request_irq(button_irqs[1].irq, tq2440_buttons_irq, button_irqs[1].flags, button_irqs[1].name, &pins_desc[1]);
	if (err < 0){
		printk("failed request irq_eint1 on %s,in line:%d\n", __FUNCTION__, __LINE__);
		goto request_irq_eint1_error;
	}
	err = request_irq(button_irqs[2].irq, tq2440_buttons_irq, button_irqs[2].flags, button_irqs[2].name, &pins_desc[2]);
	if (err < 0){
		printk("failed request irq_eint2 on %s,in line:%d\n", __FUNCTION__, __LINE__);
		goto request_irq_eint2_error;
	}
	err = request_irq(button_irqs[3].irq, tq2440_buttons_irq, button_irqs[3].flags, button_irqs[3].name, &pins_desc[3]);
	if (err < 0){
...
request_irq_eint1_error:
	free_irq(button_irqs[0].irq, (void *)&pins_desc[0]);
	return err;
request_irq_eint2_error:
	free_irq(button_irqs[0].irq, (void *)&pins_desc[0]);
	free_irq(button_irqs[1].irq, (void *)&pins_desc[1]);
	return err;
request_irq_eint4_error:
	free_irq(button_irqs[0].irq, (void *)&pins_desc[0]);
	free_irq(button_irqs[1].irq, (void *)&pins_desc[1]);
	free_irq(button_irqs[2].irq, (void *)&pins_desc[2]);
	return err;
